...
class PCVRHyFormerRankingTrainer:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_time_stats: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            # Stack per-domain temporal statistics: (B, 3)
            last_diff = device_batch.get(f'{domain}_last_action_diff', torch.zeros(B, dtype=torch.float32, device=self.device))
            gap_mean = device_batch.get(f'{domain}_time_gap_mean', torch.zeros(B, dtype=torch.float32, device=self.device))
            gap_std = device_batch.get(f'{domain}_time_gap_std', torch.zeros(B, dtype=torch.float32, device=self.device))
            seq_time_stats[domain] = torch.stack([last_diff, gap_mean, gap_std], dim=1)
        global_time_feats = device_batch.get(
            'global_time_feats',
            torch.zeros(B, 2, dtype=torch.float32, device=self.device))
        return ModelInput(
            ...
            global_time_feats=global_time_feats,
            seq_time_stats=seq_time_stats,
        )
