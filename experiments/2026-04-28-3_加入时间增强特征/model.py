...
class ModelInput(NamedTuple):
    ...
    global_time_feats: torch.Tensor  # [B, 2] (hour_of_day, day_of_week)
    seq_time_stats: dict  # {domain: tensor [B, 3]} (last_action_diff, time_gap_mean, time_gap_std)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    When ``use_inter_seq_attn=True``, an additional inter-sequence cross-attention
    step is inserted after the independent Query Decoding: each sequence's Q tokens
    attend to the concatenated encoded tokens of all *other* sequences, enabling
    explicit information exchange across domains.
    """
    ...
    def __init__(
        ...
        # Inter-sequence cross-attention: Q from seq_i, KV from all other seqs
        if use_inter_seq_attn:
            self.inter_seq_attns = nn.ModuleList([
                CrossAttention(
                    d_model=d_model,
                    num_heads=num_heads,
                    dropout=dropout,
    ...
    def forward(
        ...
        # 2.5 Inter-Sequence Cross-Attention (optional)
        if self.use_inter_seq_attn:
            inter_decoded_qs = []
            for i in range(S):
                # Concatenate all other sequences' encoded tokens and masks
                other_seqs = []
                other_masks = []
                for j in range(S):
                    if i != j:
                        other_seqs.append(next_seqs[j])
                        other_masks.append(next_masks[j])
                # (B, sum(L_j), D) and (B, sum(L_j))
                inter_kv = torch.cat(other_seqs, dim=1)
                inter_mask = torch.cat(other_masks, dim=1)
                inter_q_i = self.inter_seq_attns[i](
                    decoded_qs[i], inter_kv, inter_mask,
                    # RoPE is skipped for inter-seq attention because
                    # concatenated sequences have heterogeneous lengths/domains.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Global time features (hour_of_day, day_of_week) -> d_model bias
        self.global_time_proj = nn.Sequential(
            nn.Linear(2, d_model),
            nn.LayerNorm(d_model),
        )
        # Per-domain sequence time statistics (last_action_diff, time_gap_mean, time_gap_std) -> d_model bias
        self.seq_time_stat_projs = nn.ModuleDict({
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                use_inter_seq_attn=use_inter_seq_attn,
            )
    ...
    def _embed_seq_domain(
        ...
        time_stats: Optional[torch.Tensor] = None,
        time_stat_proj: Optional[nn.Module] = None,
    ) -> torch.Tensor:
        ...
        # Add per-domain temporal statistics as a global bias
        if time_stats is not None and time_stat_proj is not None:
            time_bias = time_stat_proj(time_stats).unsqueeze(1)  # (B, 1, D)
            token_emb = token_emb + time_bias
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                inputs.seq_time_stats.get(domain),
                self.seq_time_stat_projs[domain])
        ...
        # 5. Add global temporal bias
        if inputs.global_time_feats is not None:
            time_bias = self.global_time_proj(inputs.global_time_feats)
            output = output + time_bias
