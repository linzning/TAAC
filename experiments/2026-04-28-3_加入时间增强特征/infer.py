...
def _batch_to_model_input(
    ...
    for domain in seq_domains:
        ...
        last_diff = device_batch.get(f'{domain}_last_action_diff', torch.zeros(B, dtype=torch.float32, device=device))
        gap_mean = device_batch.get(f'{domain}_time_gap_mean', torch.zeros(B, dtype=torch.float32, device=device))
        gap_std = device_batch.get(f'{domain}_time_gap_std', torch.zeros(B, dtype=torch.float32, device=device))
        seq_time_stats[domain] = torch.stack([last_diff, gap_mean, gap_std], dim=1)
    global_time_feats = device_batch.get(
        'global_time_feats',
        torch.zeros(B, 2, dtype=torch.float32, device=device))
