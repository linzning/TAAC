...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            time_bucket[:] = 0
            last_action_diff = np.zeros(B, dtype=np.float32)
            time_gap_mean = np.zeros(B, dtype=np.float32)
            time_gap_std = np.zeros(B, dtype=np.float32)
            if ts_ci is not None:
                ...
                for i in range(B):
                    ...
                    if seq_len > 0:
                        last_ts = ts_padded[i, seq_len - 1]
                        last_action_diff[i] = float(timestamps[i] - last_ts)
                        if seq_len >= 2:
                            valid_ts = ts_padded[i, :seq_len].astype(np.float32)
                            gaps = np.diff(valid_ts)
                            gaps = np.maximum(gaps, 0.0)
                            time_gap_mean[i] = float(gaps.mean())
                            if gaps.shape[0] >= 2:
                                time_gap_std[i] = float(gaps.std(ddof=0))
                            else:
                                time_gap_std[i] = 0.0

            result[f'{domain}_time_bucket'] = torch.from_numpy(time_bucket.copy())
            result[f'{domain}_last_action_diff'] = torch.from_numpy(last_action_diff.copy())
            result[f'{domain}_time_gap_mean'] = torch.from_numpy(time_gap_mean.copy())
            result[f'{domain}_time_gap_std'] = torch.from_numpy(time_gap_std.copy())

        # Global temporal features
        hour_of_day = ((timestamps % 86400) // 3600).astype(np.float32)
        day_of_week = ((timestamps // 86400) % 7).astype(np.float32)
        global_time_feats = np.stack([hour_of_day, day_of_week], axis=1)  # (B, 2)
        result['global_time_feats'] = torch.from_numpy(global_time_feats)
