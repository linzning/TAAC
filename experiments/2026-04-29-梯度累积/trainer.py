...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # 梯度累积步数：每累积 gradient_accumulation_steps 个 micro-batch 的梯度后，
        # 才执行一次 optimizer.step() + zero_grad()。
        # 等效 batch_size = batch_size * gradient_accumulation_steps，但显存占用不变。
        # 值为 1 时退化为普通训练（每个 micro-batch 都更新参数）。
    ...
    def train(self) -> None:
        ...
        梯度累积机制:
        当 gradient_accumulation_steps > 1 时，训练循环每 N 个 micro-batch 才执行一次
        optimizer.step() + zero_grad()。total_step 记录的是 optimizer step 次数
        （而非 micro-batch 次数），确保验证频率和 early stopping 行为与不累积时一致。
        """
        print("Start training (PCVRHyFormer)")
        self.model.train()
        total_step = 0  # 记录 optimizer step 次数（梯度累积时 = micro_batch_step // accumulation_steps）
        accum_step = 0  # 当前累积周期内已处理的 micro-batch 计数
        accum_loss_sum = 0.0  # 当前累积周期内的 loss 累加（用于计算平均 loss 写入 TensorBoard）

        # 梯度累积时，在每个累积周期的起始步清零梯度
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                # 判断当前 micro-batch 是否为累积中间步（仅 forward+backward，不更新参数）
                is_accum = (self.gradient_accumulation_steps > 1)
                loss = self._train_step(batch, is_accumulation_step=is_accum)
                accum_step += 1
                loss_sum += loss
                accum_loss_sum += loss

                # 梯度累积完成一个完整周期：执行 clip + optimizer.step + zero_grad
                if self.gradient_accumulation_steps > 1 and accum_step % self.gradient_accumulation_steps == 0:
                ...
                # total_step 按 optimizer step 计数：
                # - 无累积时，每个 micro-batch 就是一次 optimizer step
                # - 有累积时，每 gradient_accumulation_steps 个 micro-batch 才是一次 optimizer step
                need_log = False  # 是否需要向 TensorBoard 写入本步 loss
                if self.gradient_accumulation_steps == 1:
                    total_step += 1
                    need_log = True
                elif accum_step == 0:
                    # 刚完成一次累积周期（accum_step 已被重置为 0），视为一次 optimizer step
                    total_step += 1
                    need_log = True

                # TensorBoard 日志：
                # - 无累积时，每个 micro-batch 记录一次 loss
                # - 有累积时，每个累积周期（optimizer step）记录一次平均 loss
                #   避免同一 step 多次写入导致覆盖问题
                if need_log and self.writer:
                    if self.gradient_accumulation_steps > 1:
                        avg_loss = accum_loss_sum / self.gradient_accumulation_steps
                        self.writer.add_scalar('Loss/train', avg_loss, total_step)
                        accum_loss_sum = 0.0
                    else:
                        self.writer.add_scalar('Loss/train', loss, total_step)

                # 显示等效 batch 大小（累积时标注累积进度）
                if self.gradient_accumulation_steps > 1:
                    train_pbar.set_postfix({"loss": f"{loss:.4f}", "accum": f"{accum_step}/{self.gradient_accumulation_steps}"})
                else:
                    train_pbar.set_postfix({"loss": f"{loss:.4f}"})

                # Step-level validation (only when eval_every_n_steps > 0).
                # 注意: total_step 按 optimizer step 计数，确保验证频率与等效 batch 对齐。
    ...
    def _train_step(self, batch: Dict[str, Any], is_accumulation_step: bool = False) -> float:
        """Run a single training step and return the scalar loss value.

        梯度累积模式说明:
        - 当 gradient_accumulation_steps > 1 时，每个 micro-batch 只做 forward + backward，
          梯度自动累加到 .grad 中。累积完成后由 train() 循环统一执行 clip + optimizer.step + zero_grad。
        - 当 gradient_accumulation_steps == 1 时，直接在内部完成 clip + step + zero_grad，
          行为与原始训练流程完全一致。

        Args:
            batch: 一个 micro-batch 的数据字典。
            is_accumulation_step: 是否处于梯度累积中间步（True 表示仅 forward+backward，
                不执行 optimizer.step）。当 gradient_accumulation_steps > 1 时为 True。

        Returns:
            当前 micro-batch 的标量 loss 值（已除以 gradient_accumulation_steps 以保证
            累积后的梯度量级与不累积时一致）。
        """
        device_batch = self._batch_to_device(batch)
        label = device_batch['label'].float()

        # 梯度累积时，zero_grad 仅在累积周期的第一步之前执行（由 train() 循环控制），
        # 此处不再每次 step 都清零，以保留跨 micro-batch 的梯度累加。
        ...
        # 梯度累积时，将 loss 除以累积步数，使得累积后的梯度与使用大 batch 时等价。
        # 这是梯度累积的标准做法，保证不同累积步数下学习率的行为一致。
        if self.gradient_accumulation_steps > 1:
        ...
        # 返回原始（未除以累积步数的）loss，便于日志展示和对比。
        return loss.item() * (self.gradient_accumulation_steps if self.gradient_accumulation_steps > 1 else 1)
