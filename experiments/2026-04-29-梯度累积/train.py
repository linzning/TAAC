...
def build_feature_specs(
    ...
    该函数将 FeatureSchema 中记录的每个特征（或特征组）转换为构建 Embedding 层
    所需的规格元组 (vocab_size, offset, length)。

    Args:
        schema: 特征模式对象，其 entries 列表每个元素为 (feature_id, offset, length)。
                - offset: 该特征（或特征组）在全局展平特征向量中的起始索引。
                - length: 该特征占据的连续位置数（例如 multi-hot 编码时 length > 1）。
        per_position_vocab_sizes: 全局展平后每个位置（slot）对应的词表大小列表。
                                  当 length > 1 时，该特征组可能包含多个 slot，
                                  各自对应不同的词表大小。

    Returns:
        按 schema.entries 顺序排列的 feature_specs 列表，
        每个元素为 (vocab_size, offset, length)。
        其中 vocab_size 取该特征组所有 slot 中最大的词表大小，
        以便统一 Embedding 表的尺寸（后续通常会对每个 slot 做取模或独立映射）。
    """
    specs: List[Tuple[int, int, int]] = []
    for fid, offset, length in schema.entries:
        # 对于占据多个 slot 的特征（如 multi-hot），取该区间内最大的词表大小。
        # 这样做的原因是：在后续 Embedding 初始化时，通常需要一个统一的 vocab_size
        # 来创建 Embedding 表；各个 slot 的实际取值会通过取模或独立索引映射到该表内。
...
def parse_args() -> argparse.Namespace:
    ...
    2. 训练超参数 (Training Hyperparameters)
       ...
       - gradient_accumulation_steps : 梯度累积步数。每累积 N 个 micro-batch 的梯度后才执行
         一次 optimizer.step()。等效 batch_size = batch_size * N，但显存占用不变。默认 1（不累积）。
    ...
    parser.add_argument('--gradient_accumulation_steps', type=int, default=1,
                        help='Number of gradient accumulation steps before performing '
                             'an optimizer step. Effective batch size = batch_size * '
                             'gradient_accumulation_steps. Default 1 (no accumulation).')
...
def main() -> None:
    ...
    # ckpt_dir:  模型检查点保存目录
    # log_dir:   文本日志输出目录
    # tf_events_dir: TensorBoard 事件文件目录（用于可视化训练曲线）
    Path(args.ckpt_dir).mkdir(parents=True, exist_ok=True)
    Path(args.log_dir).mkdir(parents=True, exist_ok=True)
    Path(args.tf_events_dir).mkdir(parents=True, exist_ok=True)

    # ---------- 2. 初始化随机种子与日志系统 ----------
    set_seed(args.seed)  # 保证实验可复现
    create_logger(os.path.join(args.log_dir, 'train.log'))  # 初始化文件日志
    logging.info(f"Args: {vars(args)}")  # 记录所有超参数，便于后续排查与复现

    # ---------- 3. 初始化 TensorBoard 写入器 ----------
    # SummaryWriter 会将训练过程中的标量（如 loss、AUC）写入事件文件，
    # 启动 tensorboard --logdir=<tf_events_dir> 即可实时查看训练曲线。
    ...
    # 解析各序列域的最大长度覆盖，例如 "seq_a:256,seq_b:256,seq_c:512,seq_d:512"
    # 这决定了不同行为序列在输入模型前被截断或填充到的长度。
    seq_max_lens = {}
    ...
    # 调用 get_pcvr_data 构建 Parquet 格式的 IterableDataset 与 DataLoader
    # - train_loader / valid_loader: 训练与验证数据迭代器
    # - pcvr_dataset: 包含 schema、词表大小、序列域定义等元信息的数据集对象
    ...
    # ---------- 5. 构建 NS (Non-Sequential) 特征分组 ----------
    # NS 特征指用户/物品的静态离散特征（如年龄、类目等），
    # 这里根据 ns_groups_json 将多个相关特征聚合为一个 "group"，
    # 便于后续通过 tokenizer 压缩成固定数量的 token 输入 Transformer。
    if args.ns_groups_json and os.path.exists(args.ns_groups_json):
        ...
        # 建立 feature_id -> schema 中索引的映射，用于将 JSON 中的特征名转为内部索引
    ...
    # ---------- 6. 构建模型 ----------
    # feature_specs: 每个 NS 特征的 (vocab_size, offset, length) 元组列表
    # 用于初始化 Embedding 层，确保各特征在 Embedding 表中的维度与位置正确。
    ...
    # 计算并打印模型结构统计信息：
    # - num_sequences: 序列域数量（如点击、加购、支付等）
    # - num_ns: 经 tokenizer 压缩后得到的 NS token 总数
    # - T: 最终输入 Transformer 的总 token 数 = query tokens * 序列域数 + NS tokens
    ...
    # ---------- 7. 训练准备 ----------
    # EarlyStopping: 当验证指标连续 patience 轮未提升时自动停止训练，
    # 并保存历史最优模型。placeholder 路径会在 trainer 内部根据 ckpt_params 重命名。
    ...
    # ckpt_params 用于构建带超参数信息的检查点子目录名，便于实验管理
    ckpt_params = {
    ...
    # 实例化训练器，封装了优化器构建、训练循环、验证、日志与模型保存逻辑
    ...
    # ---------- 8. 启动训练循环 ----------
    # train() 内部会逐 epoch 迭代 train_loader，按 eval_every_n_steps 频率
    # 在 valid_loader 上评估模型，并调用 early_stopping 判断是否提前终止。
    trainer.train()
    writer.close()  # 关闭 TensorBoard 写入器，确保所有事件刷盘
