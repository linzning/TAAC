...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # AMP dtype 选择: 优先使用 bfloat16（指数范围与 fp32 一致，不易溢出），
        # 仅当 GPU 不支持 bf16 时回退到 float16 + GradScaler。
        # bfloat16 不需要 GradScaler（不会出现 underflow/overflow），
        # float16 需要 GradScaler 防止梯度 underflow，但模型中间值可能超出 fp16 范围（±65504）
        # 导致 inf → NaN → 训练崩溃。
        ...
        if use_amp and device == 'cuda':
            if not torch.cuda.is_bf16_supported():
                logging.warning(
                    "GPU does not support bfloat16, falling back to float16. "
                    "This may cause numerical instability (inf/nan) for some models."
                )
                self._amp_dtype = torch.float16
                self._use_scaler = True
        self.scaler = torch.amp.GradScaler('cuda', enabled=use_amp and self._use_scaler)

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"use_amp={use_amp}, amp_dtype={self._amp_dtype}, "
                     f"use_scaler={self._use_scaler}")
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # autocast 仅包裹模型前向传播；loss 计算在 autocast 外以 fp32 精度执行。
        # bfloat16 模式下无需 GradScaler；float16 模式下需要 GradScaler 防止梯度 underflow，
        # 但 loss 仍需在 fp32 下计算以避免 fp16 × scaler_scale 溢出。
        with torch.amp.autocast('cuda', dtype=self._amp_dtype, enabled=self.use_amp):
            logits = self.model(model_input)  # (B, 1)

        logits = logits.squeeze(-1).float()  # 确保转换为 fp32 后再计算 loss
        ...
        if self._use_scaler:
            ...
            scaler_skipped = not self.scaler.step(self.dense_optimizer)
            if self.sparse_optimizer is not None:
                if not self.scaler.step(self.sparse_optimizer):
                    scaler_skipped = True
            self.scaler.update()
            if scaler_skipped:
                logging.warning(
                    f"GradScaler skipped optimizer step "
                    f"(scale={self.scaler.get_scale():.1f}), "
                    f"possible inf/nan gradients"
                )
        else:
            # bfloat16 / no-amp: 直接 backward + clip + step，无需 GradScaler
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0, foreach=False)
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        if len(self.valid_loader) == 0:
            logging.info("Skipping validation: empty validation loader")
            return 0.0, 0.0
    ...
    def _evaluate_step(
        ...
        logits = logits.squeeze(-1).float()  # 确保返回 fp32，避免后续 sigmoid/concat 出错
